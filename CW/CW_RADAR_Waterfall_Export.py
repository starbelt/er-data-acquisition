...
class Window(QMainWindow):
    ...
    def end_program(self):
        """ Gracefully shutsdown the program and Pluto and exports data to CSV
        Returns:
            None
        """
        my_sdr.tx_destroy_buffer()  # Destroy Tx buffer
        export_fft_data_to_csv()  # Export stored FFT data to CSV
...
def store_fft_data(freq, s_dbfs):
    ...
    for f, mag in zip(freq, s_dbfs):
        fft_data_list.append([current_time, time_since_start, f, mag])

def export_fft_data_to_csv():
    """ Exports the stored FFT data to a CSV file
    Returns:
        None
    """
...
def update():
    ...
    store_fft_data(freq, s_dbfs)  # Store FFT data in the list
